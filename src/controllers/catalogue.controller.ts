...
export async function transcribeVoiceToCatalogueItems(
    req: Request,
    res: Response,
) {
    const file = req.file;
    ...
    try {
        ...
        return res.status(400).json({
            status: "fail",
            message: "Failed To Save File To Temporary Directory",
        });
    }
    ...
    try {
        ...
        return res.status(200).json({
            status: "success",
            message: "Transcribed Catalogue Items Successfully",
            data: inventory,
        });
    } catch (error) {
        ...
        return res
            .status(400)
            .json({ status: "fail", message: "Failed To Transcribe Audio" });
    }
}
